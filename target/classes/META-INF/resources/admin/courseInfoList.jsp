...
<head>
    ...
    <title>修改课程信息界面</title>
</head>
<script>
    function deleteUser(cid, currentPage) {
        //用户安全提示
        if (confirm("您确定要删除吗？")) {
            //访问路径
            location.href = "${pageContext.request.contextPath}/deleteCourseServlet?cid=" + cid + "&currentPage=" + currentPage;
        }
    }
</script>
<body>
...
<div>
    ...
    <div style="float:left;width: 85%;">
        ...
        <span style="text-align: center; display: block;font-family:黑体;font-size: 1.5em;">课程信息详情</span>
        <div class="blank" style="height: 10px;"></div>
        <div class="container">
            <div style="float: left;margin-top: 20px">
                <form class="form-inline" action="${pageContext.request.contextPath}/courseListServlet"
                      method="post">
                    ...
                    <div class="form-group">
                        <label for="exampleInputName3">课程号</label>
                        <input type="text" name="courseId" value="${condition.courseId[0]}" class="form-control"
                               id="exampleInputName3">
                    </div>

                    <div class="form-group">
                        <label for="exampleInputEmail2">任课老师</label>
                    ...
                    <tr align="center">
                        ...
                        <th>
                            <input type="submit" value="修改"
                                   onclick="window.location.href = '${pageContext.request.contextPath}/returnOneCourseServlet?cid=${t.courseId}'"
                                   style="border:1px solid rgb(154, 240, 154);">
                            <input type="button" value="删除"
                                   onclick="window.location.href = 'javascript:deleteUser(${t.courseId},${pb.currentPage});'"
                                   style="border:1px solid rgb(248, 69, 69);">
                        </th>
                    </tr>
                </c:forEach>
            </table>
        ...
        <div style="margin-left: 15%;margin-bottom: 10%">
            <nav aria-label="Page navigation">
                <ul class="pagination pagination-lg">
                    <li>
                        <a href="${pageContext.request.contextPath}/courseListServlet?currentPage=${pb.currentPage -1}&rows=10&courseName=${condition.courseName[0]}&courseId=${condition.courseId[0]}&tName=${condition.tName[0]}"
                           aria-label="Previous">
                        ...
                        <c:if test="${pb.currentPage == i}">
                            <li class="active">
                                <a style="color: #0f0f0f;"
                                   href="${pageContext.request.contextPath}/courseListServlet?currentPage=${i}&rows=10&courseName=${condition.courseName[0]}&courseId=${condition.courseId[0]}&tName=${condition.tName[0]}">&nbsp;&nbsp;${i}&nbsp;</a>
                            </li>
                        </c:if>
                        <c:if test="${pb.currentPage != i}">
                            <li>
                                <a href="${pageContext.request.contextPath}/courseListServlet?currentPage=${i}&rows=10&courseName=${condition.courseName[0]}&courseId=${condition.courseId[0]}&tName=${condition.tName[0]}">&nbsp;&nbsp;${i}&nbsp;</a>
                            </li>
                        </c:if>
                    </c:forEach>
